...
package com.funtl.framework.apache.httpclient;
...
import com.funtl.framework.apache.httpclient.meta.http.ApplicationType;
import com.funtl.framework.apache.httpclient.meta.http.ContentEncoding;
import com.funtl.framework.apache.httpclient.meta.http.HttpResult;
import com.funtl.framework.apache.httpclient.meta.http.SimpleTrustManager;
...
public class HttpClientUtil {
	...
	public String fetchData(HttpRequestBase request) {
		...
		logger.warn("\n===============================================httpclient start ===============================================\n\t " + "返回状态码={}, 返回结果={}" + "\n===============================================httpclient end ================================================", result.getCode(), result.getResult());
		return Optional.ofNullable(result.getResult()).orElse("");
	}
	...
	public HttpResult fetchDetailData(HttpRequestBase request) {
		...
		try {
			...
			if (rsentity.getContentEncoding() != null && rsentity.getContentEncoding().getValue().toLowerCase().contains(ContentEncoding.GZIP.getName())) {
				rsentity = new GzipDecompressingEntity(rsentity);
			}
			if (response.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
				Charset rescharset = ContentType.getOrDefault(rsentity).getCharset();
				if (rescharset != null && rescharset.name().equals(HTTP.DEF_CONTENT_CHARSET)) {
				...
				logger.error("fetch request return error status:" + request.getURI() + response.getStatusLine().getStatusCode());
				result = EntityUtils.toString(rsentity);
