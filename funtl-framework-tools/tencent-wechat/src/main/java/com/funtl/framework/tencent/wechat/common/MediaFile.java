...
public class MediaFile {
	...
	private String uploadUrl() {
		String url = UPLOAD + TokenProxy.accessToken() + "&" + PARAM_TYPE + "=" + this.type.name();
		return url;
	}
	...
	private String downloadUrl() {
		String url = DOWNLOAD + TokenProxy.accessToken() + "&" + PARAM_MEDIA_ID + "=" + this.mediaId;
		return url;
	}
	...
	public String upload(File file, MediaType type) {
		...
		if (StringUtils.isNotBlank(this.mediaId)) return this.mediaId;
		return null;
	}
	...
	public byte[] download(String mediaId) {
		...
		if (fb == null || fb.length == 0) return null;
		return fb;
	}
