...
import com.funtl.framework.paypal.base.exception.InvalidResponseDataException;
...
public abstract class HttpConnection {
	...
	public InputStream executeWithStream(String url, String payload, Map<String, String> headers) throws InvalidResponseDataException, IOException, InterruptedException, HttpErrorException {
		...
		try {
			...
			String mode = ConfigManager.getInstance().getConfigurationMap().get(Constants.MODE);
			// Print if on live
			...
			do {
				try {
					...
					// FAILURE
					reader = new BufferedReader(new InputStreamReader(connection.getInputStream(), Constants.ENCODING_FORMAT));
					...
					try {
						...
						if (connection.getErrorStream() != null) {
							reader = new BufferedReader(new InputStreamReader(connection.getErrorStream(), Constants.ENCODING_FORMAT));
							errorResponse = read(reader);
