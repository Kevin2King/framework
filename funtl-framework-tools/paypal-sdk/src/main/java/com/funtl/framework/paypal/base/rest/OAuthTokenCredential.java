...
import com.funtl.framework.paypal.base.codec.binary.Base64;
...
import com.funtl.framework.paypal.base.util.UserAgentHeader;
...
public final class OAuthTokenCredential {
	 ...
	 * "/v1/oauth2/token"
	 *
	 * @param oauthTokenPath the URI part to set
	 */
	 ...
	 * @param clientID         Client ID for the OAuth
	 * @param clientSecret     Client Secret for OAuth
	 * @param configurationMap Dynamic configuration map which should have an entry for
	...
	private synchronized void generateAccessToken() throws PayPalRESTException {
		...
		try {
			...
			UserAgentHeader userAgentHeader = new UserAgentHeader(sdkVersion != null ? sdkVersion.getSDKId() : null, sdkVersion != null ? sdkVersion.getSDKVersion() : null);
	...
	private HttpConfiguration getOAuthHttpConfiguration() throws MalformedURLException {
		...
		if (Boolean.parseBoolean(configurationMap.get(Constants.USE_HTTP_PROXY))) {
			...
			String proxyUserName = configurationMap.get(Constants.HTTP_PROXY_USERNAME);
			String proxyPassword = configurationMap.get(Constants.HTTP_PROXY_PASSWORD);
