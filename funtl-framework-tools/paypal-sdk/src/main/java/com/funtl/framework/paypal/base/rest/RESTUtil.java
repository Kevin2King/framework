...
import com.funtl.framework.paypal.api.openidconnect.UserinfoParameters;
...
public final class RESTUtil {
	 ...
	 *
	 * @param pattern    URI pattern with place holders for replacement strings
	 * @param parameters Replacement objects
	...
	public static String formatURIPath(String pattern, Object[] parameters) {
		...
		if (pattern != null) {
			if (parameters != null && parameters.length == 1 && parameters[0] instanceof CreateFromAuthorizationCodeParameters) {
				...
				finalParameters = splitParameters(pattern, ((CreateFromAuthorizationCodeParameters) parameters[0]).getContainerMap());
			} else if (parameters != null && parameters.length == 1 && parameters[0] instanceof CreateFromRefreshTokenParameters) {

				// Form a object array using the passed
				// CreateFromRefreshTokenParameters
				finalParameters = splitParameters(pattern, ((CreateFromRefreshTokenParameters) parameters[0]).getContainerMap());
			} else if (parameters != null && parameters.length == 1 && parameters[0] instanceof UserinfoParameters) {

				// Form a object array using the passed UserinfoParameters
				finalParameters = splitParameters(pattern, ((UserinfoParameters) parameters[0]).getContainerMap());
			} else if (parameters != null && parameters.length == 1 && parameters[0] instanceof QueryParameters) {

				// Form a object array using the passed UserinfoParameters
				finalParameters = splitParameters(pattern, ((QueryParameters) parameters[0]).getContainerMap());
			} else if (parameters != null && parameters.length == 1 && parameters[0] instanceof Map<?, ?>) {

				// Form a object array using the passed Map
				finalParameters = splitParameters(pattern, ((Map<?, ?>) parameters[0]));
			} else {
	 ...
	 * @throws PayPalRESTException
	 */
	public static String formatURIPath(String pattern, Map<String, String> queryParameters, String... pathParameters) throws PayPalRESTException {
				...
				for (String q : queries) {
					...
					if (params != null && params.length == 2) {
						...
						if (containerMap.containsKey(key)) {
							...
							try {
								objectList.add(URLEncoder.encode((String) object, Constants.ENCODING_FORMAT));
							} catch (UnsupportedEncodingException e) {
								// Ignore
							}
						} else {
