...
import com.funtl.framework.alipay.trade.pay.protocol.downloadbill.internal.parser.xml.ObjectXmlParser;
import com.funtl.framework.alipay.trade.pay.protocol.downloadbill.internal.util.AlipayHashMap;
import com.funtl.framework.alipay.trade.pay.protocol.downloadbill.internal.util.AlipayLogger;
import com.funtl.framework.alipay.trade.pay.protocol.downloadbill.internal.util.AlipaySignature;
import com.funtl.framework.alipay.trade.pay.protocol.downloadbill.internal.util.AlipayUtils;
import com.funtl.framework.alipay.trade.pay.protocol.downloadbill.internal.util.RequestParametersHolder;
import com.funtl.framework.alipay.trade.pay.protocol.downloadbill.internal.util.StringUtils;
import com.funtl.framework.alipay.trade.pay.protocol.downloadbill.internal.util.WebUtils;
		...
		try {
			...
			if (!StringUtils.isEmpty(this.alipayPublicKey)) {
				...
				if (tRsp.isSuccess() || (!tRsp.isSuccess() && !StringUtils.isEmpty(signItem.getSign()))) {
					...
					boolean rsaCheckContent = AlipaySignature.rsaCheckContent(signItem.getSignSourceDate(), signItem.getSign(), this.alipayPublicKey, this.charset);
	...
	public <T extends AlipayResponse> Map<String, Object> doPost(AlipayRequest<T> request, String accessToken) throws AlipayApiException {
		...
		try {
			if (request instanceof AlipayUploadRequest) {
				...
				rsp = WebUtils.doPost(urlSb.toString(), appParams, fileParams, charset, connectTimeout, readTimeout);
			} else {
